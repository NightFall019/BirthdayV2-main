...
let fireflies = [], shootingStars = [], fwParticles = [], fwLaunchers = [];
...
function buildScene(){
  ...
  fireflies = []; shootingStars = []; fwParticles = []; fwLaunchers = []; lastShootingStar = 0;
  buildHeartPoly();
  ...
  /* fireflies — dreamy glowing orbs that drift around the tree */
  for (let i = 0; i < 18; i++){
    fireflies.push({
      x: rand(cx - rx * 1.3, cx + rx * 1.3),
      y: rand(cy - ry * 1.1, cy + ry * 1.3),
      phase: rand(0, 6.28), pulsePhase: rand(0, 6.28),
      speed: rand(8, 20), wanderX: rand(0.3, 0.8), wanderY: rand(0.2, 0.5),
      size: rand(2.5, 6), baseAlpha: rand(0.3, 0.7),
    });
  }
}
...
/* ============================================================
   FIREFLIES — dreamy glowing orbs drifting around the tree
   ============================================================ */
function drawFireflies(t, dt){
  const appear = clamp01((t - 0.3) / 1.5);
  if (appear <= 0) return;
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
  for (const f of fireflies){
    f.x += Math.sin(t * f.wanderX + f.phase) * f.speed * dt;
    f.y += Math.cos(t * f.wanderY + f.phase * 1.3) * f.speed * 0.6 * dt;
    if (f.x < cx - rx * 1.5) f.x = cx + rx * 1.4;
    if (f.x > cx + rx * 1.5) f.x = cx - rx * 1.4;
    if (f.y < cy - ry * 1.5) f.y = cy + ry * 1.4;
    if (f.y > groundY) f.y = cy - ry;
    const pulse = 0.5 + 0.5 * Math.sin(t * 2.5 + f.pulsePhase);
    const alpha = f.baseAlpha * pulse * appear;
    const sz = f.size * (0.8 + 0.4 * pulse);
    const g = ctx.createRadialGradient(f.x, f.y, 0, f.x, f.y, sz * 3.5);
    g.addColorStop(0, `rgba(255,230,150,${alpha})`);
    g.addColorStop(0.3, `rgba(255,200,100,${alpha * 0.5})`);
    g.addColorStop(1, 'rgba(255,180,80,0)');
    ctx.fillStyle = g;
    ctx.fillRect(f.x - sz * 3.5, f.y - sz * 3.5, sz * 7, sz * 7);
    ctx.globalAlpha = alpha;
    ctx.fillStyle = '#fffbe8';
    ctx.beginPath(); ctx.arc(f.x, f.y, sz * 0.5, 0, 6.2832); ctx.fill();
  }
  ctx.restore();
}
...
function spawnShootingStar(){
  shootingStars.push({
    x: rand(W * 0.05, W * 0.95), y: rand(0, H * 0.25),
    vx: rand(300, 600) * (Math.random() < 0.5 ? 1 : -1),
    vy: rand(100, 300),
    trail: [], age: 0, life: rand(0.35, 0.7), size: rand(1.5, 3.5),
  });
}
function drawShootingStars(t, dt){
  if (t < T.bloomT0 + T.bloomSpan * 0.5) return;
  if (t - lastShootingStar > rand(3, 7)){ spawnShootingStar(); lastShootingStar = t; }
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  for (let i = shootingStars.length - 1; i >= 0; i--){
    const s = shootingStars[i]; s.age += dt;
    if (s.age >= s.life){ shootingStars.splice(i, 1); continue; }
    s.trail.push({ x: s.x, y: s.y });
    if (s.trail.length > 18) s.trail.shift();
    s.x += s.vx * dt; s.y += s.vy * dt;
    const progress = s.age / s.life, alpha = 1 - progress;
    for (let j = 0; j < s.trail.length; j++){
      const tp = s.trail[j], ta = (j / s.trail.length) * alpha * 0.6;
      ctx.globalAlpha = ta; ctx.fillStyle = '#fffbe8';
      ctx.beginPath(); ctx.arc(tp.x, tp.y, s.size * (j / s.trail.length), 0, 6.2832); ctx.fill();
    }
    const hg = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, s.size * 5);
    hg.addColorStop(0, `rgba(255,255,255,${alpha})`);
    hg.addColorStop(0.3, `rgba(255,240,200,${alpha * 0.6})`);
    hg.addColorStop(1, 'rgba(255,220,180,0)');
...
/* ============================================================
   HEART FIREWORKS — tap anywhere after tree is done
   ============================================================ */
function launchFirework(sx, sy){
  const targetY = rand(H * 0.08, H * 0.35);
  fwLaunchers.push({
    x: sx, y: sy, targetY,
    vy: -(sy - targetY) * 2.4,
    trail: [], age: 0, exploded: false,
    color: pick(['#ff0055', '#ff2e63', '#d90429', '#ff007f', '#8338ec', '#ffb703'])
  });
}
function heartFireworkBurst(bx, by){
  const colors = ['#ff0055', '#ff2e63', '#e60049', '#ff007f', '#d90429', '#ff5964', '#8338ec', '#ffb703', '#ff4d6d'];
  const count = 55, baseSpeed = rand(90, 150);
  for (let i = 0; i < count; i++){
    const a = (i / count) * Math.PI * 2;
    const hx = 16 * Math.pow(Math.sin(a), 3);
    const hy = -(13 * Math.cos(a) - 5 * Math.cos(2 * a) - 2 * Math.cos(3 * a) - Math.cos(4 * a));
    const mag = Math.hypot(hx, hy) || 1;
    const speed = baseSpeed * rand(0.8, 1.2);
    fwParticles.push({
      x: bx, y: by,
      vx: (hx / mag) * speed + rand(-12, 12),
      vy: (hy / mag) * speed + rand(-12, 12),
      size: rand(2.5, 5.5), color: pick(colors),
      alpha: 1, age: 0, life: rand(0.9, 1.8),
      gravity: rand(45, 85), isHeart: Math.random() < 0.4,
    });
  }
  for (let i = 0; i < 22; i++){
    const ang = rand(0, 6.2832), speed = baseSpeed * rand(0.25, 0.65);
    fwParticles.push({
      x: bx, y: by,
      vx: Math.cos(ang) * speed, vy: Math.sin(ang) * speed,
      size: rand(2, 4), color: pick(['#ff0055', '#ff2e63', '#ff007f', '#ffb703']),
      alpha: 1, age: 0, life: rand(0.4, 1.1),
      gravity: rand(18, 45), isHeart: false,
    });
  }
}
function drawFWHeart(hx, hy, s, col, a){
  ctx.save(); ctx.globalAlpha = a; ctx.fillStyle = col;
  heartShape(ctx, hx, hy - s * 0.5, s, s);
  ctx.fill(); ctx.restore();
}
function updateFireworks(t, dt){
  /* launchers rising up */
  for (let i = fwLaunchers.length - 1; i >= 0; i--){
    const l = fwLaunchers[i]; l.age += dt;
    l.trail.push({ x: l.x, y: l.y });
    if (l.trail.length > 10) l.trail.shift();
    l.y += l.vy * dt; l.vy += 180 * dt;
    if (l.y <= l.targetY && !l.exploded){
      l.exploded = true; heartFireworkBurst(l.x, l.y);
      fwLaunchers.splice(i, 1); continue;
    }
    if (l.age > 3){ fwLaunchers.splice(i, 1); continue; }
    ctx.save();
    for (let j = 0; j < l.trail.length; j++){
      const tp = l.trail[j], ta = (j / l.trail.length) * 0.85;
      const tg = ctx.createRadialGradient(tp.x, tp.y, 0, tp.x, tp.y, 4.5);
      tg.addColorStop(0, l.color); tg.addColorStop(1, 'rgba(255,0,85,0)');
      ctx.globalAlpha = ta; ctx.fillStyle = tg; ctx.fillRect(tp.x - 4.5, tp.y - 4.5, 9, 9);
    }
    const lg = ctx.createRadialGradient(l.x, l.y, 0, l.x, l.y, 7);
    lg.addColorStop(0, l.color); lg.addColorStop(0.6, '#ff0055'); lg.addColorStop(1, 'rgba(255,0,85,0)');
    ctx.globalAlpha = 1; ctx.fillStyle = lg; ctx.fillRect(l.x - 7, l.y - 7, 14, 14);
    ctx.restore();
  }
  /* explosion particles */
  ...
  for (let i = fwParticles.length - 1; i >= 0; i--){
    const p = fwParticles[i]; p.age += dt;
    if (p.age >= p.life){ fwParticles.splice(i, 1); continue; }
    p.vy += p.gravity * dt; p.x += p.vx * dt; p.y += p.vy * dt; p.vx *= 0.985;
    const prog = p.age / p.life; p.alpha = 1 - prog * prog;
    if (p.isHeart){
      drawFWHeart(p.x, p.y, p.size * 2.4, p.color, p.alpha);
    } else {
      ctx.globalAlpha = p.alpha; ctx.fillStyle = p.color;
      ctx.beginPath(); ctx.arc(p.x, p.y, p.size * (1 - prog * 0.3), 0, 6.2832); ctx.fill();
    }
  }
  ctx.restore();
}
...
function fire(){
  ...
  bgMusic.volume = 0.3;
  bgMusic.play().catch(() => {});
  window._musicTimer = setTimeout(() => {
    bgMusic.pause();
  }, 43000);
  
  window._picTimer = setTimeout(() => {
    document.getElementById('revealPic').classList.add('show');
  }, 12000); // adjust: how many ms after firing you want it to appear

  window._msgTimer = setTimeout(() => {
    document.querySelectorAll('.galleryPic').forEach(pic => pic.classList.add('show'));
    const msgText = "if love had a name, it would sound like yours";
    const msgEl = document.getElementById('revealMsg');
    msgEl.innerHTML = msgText.split(' ').map(word => `<span>${word}</span>`).join(' ');
...
function armReplay(){
  ...
  if (fwHint) fwHint.classList.add('is-shown');
}
...
function resetAll(){
  ...
  clearTimeout(window._musicTimer);
  clearTimeout(window._picTimer);
  const bgMusic = document.getElementById('bgMusic');
  bgMusic.pause();
  bgMusic.currentTime = 0;
  document.getElementById('revealPic').classList.remove('show');
  document.getElementById('revealMsg').innerHTML = '';
  clearTimeout(window._msgTimer);
  document.querySelectorAll('.galleryPic').forEach(pic => pic.classList.remove('show'));

  
  showWish(false);
  window.bdayDone = false; replayArmed = false;
  replay.classList.remove('is-shown'); replay.hidden = true;
  if (fwHint) fwHint.classList.remove('is-shown');
  fwParticles = []; fwLaunchers = []; shootingStars = [];
  if (filmTL){ filmTL.pause(0); }
  gsap.set([flood, bloom], { autoAlpha: 0 });
  gsap.set(field, { autoAlpha: 0 });
  gsap.set(arrow, { opacity: 1, scaleY: 1 });

  // Hide LISHY surprise elements
  document.querySelectorAll('.name-char').forEach(el => el.classList.remove('is-visible'));
  const lishySub = $('lishySub');
  if (lishySub) lishySub.classList.remove('is-visible');
...
if (reduceMotion){
  ...
  /* --- magical cursor / touch trail ---------------------------------------- */
  const trailContainer = $('cursorTrail');
  const trailColors = ['#ff6f97','#ffb14e','#ff8fae','#ffd36a','#e23b67','#ffc0cb','#ffe0a0'];
  let lastTrailTime = 0;
  function spawnTrailParticle(x, y){
    ...
    if (isHeart){
      el.innerHTML = '<svg viewBox="0 0 24 22" width="' + size + '" height="' + size + '"><path d="M12 20C5.5 15 1.5 11.4 1.5 6.9 1.5 3.6 4 1.5 7 1.5c2 0 3.4 1.1 5 3 1.6-1.9 3-3 5-3 3 0 5.5 2.1 5.5 5.4C23.5 11.4 19.5 15 12 20Z" fill="' + pick(trailColors) + '"/></svg>';
    } else {
      el.style.width = el.style.height = size + 'px';
      el.style.borderRadius = '50%';
      el.style.background = 'radial-gradient(circle, rgba(255,255,255,0.9), rgba(255,220,180,0) 70%)';
    }
    el.style.left = x + 'px';
    el.style.top = y + 'px';
    el.style.setProperty('--drift-x', rand(-25, 25) + 'px');
    el.style.setProperty('--drift-y', rand(-35, -8) + 'px');
    el.style.setProperty('--drift-r', rand(-60, 60) + 'deg');
  ...
  document.addEventListener('pointermove', (e) => {
    ...
    if (Math.random() < 0.4) spawnTrailParticle(e.clientX + rand(-10, 10), e.clientY + rand(-10, 10));
  });

  /* --- fireworks click handler ---------------------------------------------- */
  if (fwHint){
    fwHint.addEventListener('click', (e) => {
      e.stopPropagation();
      fwHint.classList.remove('is-shown');
      ...
      letters.forEach((l, index) => {
        let t1 = setTimeout(() => {
          launchFirework(l.x, h);
          let t2 = setTimeout(() => {
            const el = document.getElementById(l.id);
            if (el) el.classList.add('is-visible');
          }, 800); // Wait a bit for the firework to rise and explode
          lishyTimeouts.push(t2);
        }, index * 500);
        lishyTimeouts.push(t1);
      });

      let t3 = setTimeout(() => {
        const sub = document.getElementById('lishySub');
        if (sub) sub.classList.add('is-visible');
      }, letters.length * 500 + 1000);
      lishyTimeouts.push(t3);
    });
  }

  document.querySelector('.scene').addEventListener('click', (e) => {
    if (!played) return;
    if (e.target === replay || replay.contains(e.target)) return;
    if (fwHint && (e.target === fwHint || fwHint.contains(e.target))) return;
    const rect = canvas.getBoundingClientRect();
